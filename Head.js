...
function calcular(e) {
  ...
  if (tab === "alfabetos") {
    ...
    document.querySelector(
      "#restTitulo"
    ).innerText = `Resultado de la operacion: ${operation.toLowerCase()}`;
    ...
    if (operation === "Potencia")
      nPotencia = parseInt(prompt("A que potencia lo quiere elevar"));
    console.log(nPotencia);
    const res = opLenguajes(operation, a, b, nPotencia - 1);
    console.log(nPotencia);
    document.querySelector(
      "#restTitulo"
    ).innerText = `Resultado de la operacion: ${
      operation.toLowerCase() +
      (operation === "Potencia" ? " L" + nPotencia : "")
    }`;
    if (operation === "Cardinalidad")
